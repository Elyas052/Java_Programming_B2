...
public class RemoveWhileLooping {
    public static void main(String[] args) {
        ...
        // Traditional fori loop - > removing element starting from beginning is not good, because it works with indexes and once the element is removed, all other elements shift to leave
        // this causes to skip some elements in the collection.
        /*
        ...
        while (it.hasNext()) { // Checks if there is an element next to it. If not, the loop stops
            if (it.next() < 5) {  // move to the next element and the return it
                it.remove(); // remove from the list if the condition is true
            }
        }
        System.out.println(list);
