...
public class LongestPalindrome {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < arr.length; i++) {   // With this OUTER loop, I am getting each element of the arrays
            ...
            String eachWord = arr[i];  // On each cycle, here I am assigning the values of the specific index of an array to String eachWord
            ...
            for (int j = eachWord.length() - 1; j >= 0; j--) { // On each OUT loop cycle, amd also creating a new loop
                // With thi INNER Loop. I am getting each character of eachWord starting from an end
                reversed += "" + eachWord.charAt(j);      // I am adding each character into this String reversed

            }

            if (eachWord.equals(reversed)) {  // On each cycle of OUTER loop, I am comparing if eachWord is equal to the reversed to see if it is PALINDROME
                if (eachWord.length() > longestPalindrome.length()) {  // If it's PALINDROME, I am comparing the length of the longestPalindrome with the eachWord
